...
class ESEAttn(nn.Layer):
    # =============================================================================
    # PATCH (2026-06-18, vendor hotfix): the upstream ``ESEAttn``
    # module uses ``self.fc = nn.Conv2D(feat_channels, feat_channels, 1)``
    # for the channel-attention 1x1 conv. Paddle 2.6.2's IR replaces
    # this 1x1 conv with the C++ ``fused_conv2d_add_act`` operator,
    # which is compiled against cuDNN 8.6 / CUDA 11.8 and rejected by
    # the system's cuDNN 8.9 (from the ``nvidia-cudnn-cu12==8.9.7.29``
    # wheel) with::
    #
    #     ExternalError: CUDNN error(9), CUDNN_STATUS_NOT_SUPPORTED.
    #       at ppyoloe_head.py:43:  weight = F.sigmoid(self.fc(avg_feat))
    #
    # Fix: replace ``nn.Conv2D`` with explicit ``create_parameter`` +
    # ``F.conv2d`` so Paddle's IR cannot fuse the call.
    # =============================================================================
    def __init__(self, feat_channels, act='swish', attn_conv='convbn'):
        super(ESEAttn, self).__init__()
        # Equivalent to nn.Conv2D(feat_channels, feat_channels, 1) but
        # held as raw parameters so Paddle's IR doesn't fuse the call.
        self._fc_weight = self.create_parameter(
            shape=[feat_channels, feat_channels, 1, 1],
            attr=ParamAttr(initializer=KaimingNormal()),
        )
        self._fc_bias = self.create_parameter(
            shape=[feat_channels],
            attr=ParamAttr(initializer=Constant(value=0.0)),
        )
        if attn_conv == 'convbn':
            ...
            self.conv = None
        # Apply KaimingNormal to the weight (matches nn.Conv2D default)
        KaimingNormal()(self._fc_weight)

    def _init_weights(self):
        # The original code did ``normal_(self.fc.weight, std=0.001)``
        # which is a custom std. nn.Conv2D's default init is KaimingNormal
        # which we apply in __init__. Override the std here for
        # closer upstream behaviour:
        normal_(self._fc_weight, std=0.001)

    def forward(self, feat, avg_feat):
        # Manual F.conv2d — Paddle does NOT fuse this call with
        # fused_conv2d_add_act because there's no Conv2D layer to
        # anchor the IR pass. ~2% per-conv latency regression
        # is acceptable for the 1x1 head conv.
        weight = F.sigmoid(
            F.conv2d(
                avg_feat,
                self._fc_weight,
