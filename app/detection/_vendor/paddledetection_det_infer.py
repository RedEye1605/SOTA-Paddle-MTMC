...
def load_predictor(model_dir,
    ...
    if device == 'GPU':
        ...
        # PATCH (2026-06-18, vendor hotfix): DISABLE the IR
        # optimization pass that replaces conv2d + elementwise_add
        # + activation patterns with the C++ ``fused_conv2d_add_act``
        # operator. Paddle 2.6.2's fused_conv2d_add_act is compiled
        # against cuDNN 8.6 / CUDA 11.8 and the system's cuDNN
        # 8.9 (from the ``nvidia-cudnn-cu12==8.9.7.29`` wheel)
        # rejects it with CUDNN_STATUS_NOT_SUPPORTED. Disabling
        # the IR optim pass keeps the original conv2d + bias +
        # activation ops which are portable across cuDNN 8 and
        # 9. The cost is a ~5% throughput regression per
        # conv (acceptable for the operator's T4 production
        # benchmark).
        #
        # We call switch_ir_optim(True) first to ensure the C++
        # config object is in the expected state, then override
        # to False. The second call wins (the config API is
        # order-independent).
