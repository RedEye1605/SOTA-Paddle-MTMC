...
class PushStream(object):
    ...
    def initcmd(self, fps, width, height):
        self.command = [
            ...
            '-', '-pix_fmt', 'yuv420p',
            # PATCH (2026-06-15): force H.264/libx264 so MediaMTX's HLS
            # muxer instantiates. PP-Human's default (no -c:v flag) emits
            # MPEG-4 Part 2, which MediaMTX rejects for HLS remux. This
            # is a temporary vendor hotfix until the change is baked
            # into the Dockerfile or upstreamed.
            '-c:v', 'libx264',
            '-preset', 'ultrafast',
            '-tune', 'zerolatency',
            # GOP size = 2 * fps for live-stream stability: forces an
            # IDR every 0.5s so HLS segments are short and seekable.
            '-g', str(fps * 2),
            '-bf', '0',
            # PATCH (2026-06-18): force RTSP transport to TCP.
            # ffmpeg 4.4 defaults the rtsp output muxer to UDP
            # (``-rtspflags`` is unsupported in 4.4's rtsp muxer; the
            # ``-rtsp_transport`` flag is muxer-only and must be
            # placed immediately before the output URL, NOT as an
            # input option). The operator's MediaMTX at
            # ``198.51.100.20:8554`` rejects the UDP transport with
            # ``461 Unsupported Transport`` and the publish
            # SETUP handshake dies before the first IDR — the
            # ffmpeg child becomes a zombie within 100 ms of
            # ``PushStream.initcmd`` and the operator sees an
            # empty MediaMTX path. The rtsp muxer option
            # ``-rtsp_transport tcp`` is the documented ffmpeg
            # workaround (``ffmpeg -h muxer=rtsp``).
            '-rtsp_transport', 'tcp',
            '-f', 'rtsp', self.pushurl
        ]
